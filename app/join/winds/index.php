<!DOCTYPE html>
<html>
<head>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-8965430-7"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());

        gtag('config', 'UA-8965430-7');
    </script>

    <meta charset="utf-8">
    <title>Winds Auditions | Tiger Band</title>
    <link rel="stylesheet" href="/css/main.css">
    <link href="https://fonts.googleapis.com/css?family=Roboto:400,700" rel="stylesheet">
    <link rel="icon" type="image/png" sizes="32x32" href="/favicons/favicon-32x32.png">
    <link rel="icon" type="image/png" sizes="16x16" href="/favicons/favicon-16x16.png">
    <link rel="shortcut icon" href="/favicons/favicon.ico">
    <meta name=viewport content="width=device-width, initial-scale=1">
    <meta name="description" content="The Band That Shakes the Southland">
    <meta name="keywords" content="tiger, band, clemson, university, shakes, southland">
</head>

<!--- HEADER --->

<input type='checkbox' id='mobileMenu'>

<div id="mobileHeader">
	<a href="/"><img id="tbMobileLogo" src="/img/logos/TigerBandMobileLogo.png" width="120"></a>
	<label for="mobileMenu" id="menuButton"><span>=</span></label>
</div>

<label id='mobileScreenShade' for='mobileMenu'></label>

<!--- NAVIGATION --->

<nav>
    <a href="/">Home</a>
    <a href="/this_season/">This Season</a>
    <a href="/join/" class="active">Join</a>
    <a href="/bands/">Bands</a>
    <a href="/organizations/">Organizations</a>
    <a href="/staff/">Staff</a>
    <a href="/members/">Members Only</a>
    <a href="/contact/">Contact</a>
</nav>

<!--- PAGE CONTENT --->

<body>
    <div class="heading">
        <h3>Audition for</h3>
        <h4>Tiger Band Brass and Woodwinds</h4>
    </div>

    <div id="Col2FlexContent" class="about">
        <div id="col1Flex">
            <article class="text fullWidth">
                <h1 class="purpleText">2019 Tiger Band Brass and Woodwind Auditions</h1>
                <p>All auditions for wind players for the 2019 Tiger Band season will be completed by video submission. Cell phone videos are acceptable if that is the only equipment available. All of the information you need to complete and submit your video, including a video tutorial is posted below. If you still have questions, please contact the band office at 864.656.3380.</p>
                <p><a href="https://docs.google.com/forms/d/e/1FAIpQLSdqMOgrJia-sCJOXo7qGN4SLcikE00ojeCSMfPfGarcJ2OYdA/viewform?usp=sf_link" class="richLink cta">Tiger Band Application</a></p>
                <p>The first round of drumline auditions will be via video submission due on June 1. Please see the <a href="http://tband.people.clemson.edu/drumline/">Clemson University Drumline website</a> for more details.</p>
            </article>
        </div>

        <div id="col1Flex">

            <!--- DEADLINE CALENDAR UI --->

            <div class="deadlines">
                <div class="month">
                    <div class="boxUIelement sq white" id="text1" onclick="moreInfo(1)">
                        <div class="textContainer">
                            <p>July</p>
                            <b>8</b>
                            <p>Returners</p>
                        </div>
                    </div>
                    <div class="boxUIelement sq white" id="text2" onclick="moreInfo(2)">
                        <div class="textContainer">
                            <p>July</p>
                            <b>22</b>
                            <p>New Members</p>
                        </div>
                    </div>
                    <div class="boxUIelement sq white" id="text3" onclick="moreInfo(3)">
                        <div class="textContainer">
                            <p>July</p>
                            <b>29</b>
                            <p>Results</p>
                        </div>
                    </div>
                    <div class="deadlinesInfo" id="info1">
                        <p>Returners Video Audition Submission Deadline</p>
                        <p><a href="https://docs.google.com/forms/d/e/1FAIpQLSdqMOgrJia-sCJOXo7qGN4SLcikE00ojeCSMfPfGarcJ2OYdA/viewform?usp=sf_link" class="richLink cta">Tiger Band Application</a></p>
                    </div>
                    <div class="deadlinesInfo" id="info2">
                        <p>First Year Members Video Audition Submission Deadline</p>
                        <p><a href="https://docs.google.com/forms/d/e/1FAIpQLSdqMOgrJia-sCJOXo7qGN4SLcikE00ojeCSMfPfGarcJ2OYdA/viewform?usp=sf_link" class="richLink cta">Tiger Band Application</a></p>
                    </div>
                    <div class="deadlinesInfo" id="info3">
                        <p>Results will be emailed to everyone by July 29, 2019</p>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <!--- AUDITION INFORMATION --->

    <div>
        <div class="heading">
            <h4>Audition Information</h4>
        </div>
    </div>

    <div id="Col2FlexContent">
        <div id="col1Flex">
            <article class="text fullWidth">
                <h1>Content</h1>
                <p>Your audition video should contain:</p>
                <ol>
                    <li>A short introduction of yourself</li>
                    <li>The Chromatic Scale (at least 2 octaves)</li>
                    <li>Tiger Rag (one time through, stop at the end of the first ending)</li>
                    <li>Alma Mater (one time through)</li>
                </ol>
                <div id="audioContainer">
                    <div class="auditionAudio">
                        <p>Tiger Rag:</p>
                        <audio src="/audio/TigerRag.mp3" controls></audio>
                    </div>
                    <div class="auditionAudio">
                        <p>Alma Mater:</p>
                        <audio src="/audio/AlmaMater.mp3" controls></audio>
                    </div>
                </div>
                <p>Sheet music for Tiger Rag and The Alma Mater can be downloaded below.</p>
                <p>Returners: Please use the posted part for your section for the audition regardless of what part you played in previous seasons of Tiger Band.</p>
            </article>
            <article class="text fullWidth">
                <h1>Submission</h1>
                <p>Upload your video to YouTube as an “unlisted” video or to another file sharing service like Google Drive, Dropbox, or Box.</p>
                <p>Secure a download link for the file that you can share with us.</p>
                <p>Fill out the online application below:</p>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSdqMOgrJia-sCJOXo7qGN4SLcikE00ojeCSMfPfGarcJ2OYdA/viewform?usp=sf_link" class="richLink cta">Tiger Band Application</a>
                <p>Results will be communicated by July 29 via email. All accepted Tiger Band members will report to campus on <u>August 14</u> to begin our 2019 preseason camp.</p>
            </article>
        </div>

        <div id="col2Flex">
          <article class="text fullWidth">
              <h1>Tutorial</h1>
              <p>Here’s a video tutorial walking you through the audition and submission process:</p>
              <div class="iframeContainer">
                  <div class="iFrameAspectRatio">
                      <iframe width="560" height="315" src="https://www.youtube.com/embed/z2QuVMQ4B3c" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                  </div>
              </div>
          </article>
        </div>
    </div>

    <!--- SHEET MUSIC UI --->

    <div id="music">
        <div class="heading">
            <h4>Sheet Music</h4>
        </div>

        <h4 class="purpleText expand" onclick="expand(this)">Tiger Rag</h4>
        <div class="expandButton">
            <img src="/img/logos/dropdownArrow.svg" width="75">
        </div>

        <div class="fileUIcontainer">
            <a href="/doc/TigerRag/TigerRag_Piccolo.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Piccolo</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Clarinet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Clarinet</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_AltoSax.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Alto Sax</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_TenorSax.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Tenor Sax</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Trumpet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Mello.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Mellophone</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Trombone.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Baritone_BC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (BC)</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Baritone_TC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (TC)</span>
                </div>
            </a>
            <a href="/doc/TigerRag/TigerRag_Sousa.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Sousaphone</span>
                </div>
            </a>
        </div>

        <h4 class="purpleText expand" onclick="expand(this)">Armed Forces Salute</h4>
        <div class="expandButton">
            <img src="/img/logos/dropdownArrow.svg" width="75">
        </div>

        <div class="fileUIcontainer">
            <a href="/doc/ArmedForcesSalute/AF_Piccolo.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Piccolo/Flute</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Clarinet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Clarinet</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Alto.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Alto Sax</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Tenor.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Tenor Sax</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Trumpet1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 1</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Trumpet2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 2</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Trumpet3.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 3</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Mello.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Mellophone</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Trombone1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 1</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Trombone2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 2</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_BaritoneBC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (BC)</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_BaritoneTC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (TC)</span>
                </div>
            </a>
            <a href="/doc/ArmedForcesSalute/AF_Sousa.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Sousaphone</span>
                </div>
            </a>
        </div>

        <h4 class="purpleText expand" onclick="expand(this)">Stars and Stripes Forever</h4>
        <div class="expandButton">
            <img src="/img/logos/dropdownArrow.svg" width="75">
        </div>

        <div class="fileUIcontainer">
            <a href="/doc/StarsAndStripes/SS_Piccolo.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Piccolo/Flute</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Clarinet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Clarinet</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Alto.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Alto Sax</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Tenor.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Tenor Sax</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Trumpet1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 1</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Trumpet2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 2</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Trumpet3.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 3</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Mello.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Mellophone</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Trombone1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 1</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Trombone2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 2</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_BaritoneBC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (BC)</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_BaritoneTC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (TC)</span>
                </div>
            </a>
            <a href="/doc/StarsAndStripes/SS_Sousa.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Sousaphone</span>
                </div>
            </a>
        </div>

        <h4 class="purpleText expand" onclick="expand(this)">Taps</h4>
        <div class="expandButton">
            <img src="/img/logos/dropdownArrow.svg" width="75">
        </div>

        <div class="fileUIcontainer">
            <a href="/doc/Taps/Taps_Piccolo.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Piccolo/Flute</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Clarinet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Clarinet</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Alto.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Alto Sax</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Tenor.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Tenor Sax</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Trumpet1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 1</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Trumpet2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 2</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Trumpet3.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet 3</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Mello.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Mellophone</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Trombone1.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 1</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Trombone2.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone 2</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_BaritoneBC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (BC)</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_BaritoneTC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (TC)</span>
                </div>
            </a>
            <a href="/doc/Taps/Taps_Sousa.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Sousaphone</span>
                </div>
            </a>
        </div>


        <h4 class="purpleText expand" onclick="expand(this)">Alma Mater</h4>
        <div class="expandButton">
            <img src="/img/logos/dropdownArrow.svg" width="75">
        </div>

        <div class="fileUIcontainer">
            <a href="/doc/AlmaMater/AlmaMater_Piccolo.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Piccolo</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Clarinet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Clarinet</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_AltoSax.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Alto Sax</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_TenorSax.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Tenor Sax</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Trumpet.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trumpet</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Mello.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Mellophone</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Trombone.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Trombone</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Baritone_BC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (BC)</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Baritone_TC.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Baritone (TC)</span>
                </div>
            </a>
            <a href="/doc/AlmaMater/AlmaMater_Sousa.pdf" target="_blank">
                <div class="fileUIfile">
                    <span>Sousaphone</span>
                </div>
            </a>
        </div>
    </div>

<!--- FOOTER --->
<?php
    readfile("../../footer.html");
?>
</body>

<script>
    //JavaScript for Collapsible Articles
    function expand(n) {
        n.classList.toggle("clicked");
    };

    //JavaScript for More Info Under Calendar Deadlines
    var active123 = [1,1,1,1,1,1,1];
    function moreInfo(x) {
        active123[x] = 0;
        document.getElementById("text" + x).classList.toggle("clicked");
        document.getElementById("info" + x).classList.toggle("clicked");
        for(var i = 0; i < active123.length; i++) {
            if(active123[i] == 0 && i != x) {
                document.getElementById("text" + i).classList.remove("clicked");
                document.getElementById("info" + i).classList.remove("clicked");
                active123[i] = 1;
            }
        }
    }
    //Copyright Date
    var startYear = 2017;
    var currentYear = new Date().getFullYear();
    if (startYear == currentYear) {
        document.getElementById("copyright").innerHTML = "COPYRIGHT &copy " + startYear;
    }
    else {
        document.getElementById("copyright").innerHTML = "COPYRIGHT &copy " + startYear + " - " + currentYear;
    }
</script>
